// ==UserScript==
// @name            GamesOriginPlugDJSelfBot UserScript
...
(function () {
  var init = function () {
    ...
    {
      window.log('GamesOriginPlugDJSelfBot waiting for loading-box...');
  ...
  loader = function () {
    if (!loaded && trys < (timeout * 1000 / retryspeed))
    {
      window.log('GamesOriginPlugDJSelfBot loading!');
      if (document.getElementsByClassName('loading-box').length > 0) {
        setTimeout(loader, retryspeed);
      } else {
        var script = document.createElement('script');
        script.textContent = '$.getScript(\'https://rawgit.com/Hybris95/GamesOriginPlugDJSelfBot/master/gamesOriginPlugDJSelfBot.js\');';
        document.head.appendChild(script);
        window.log('GamesOriginPlugDJSelfBot loaded!');
        loaded = true;
      }
    }
  };
